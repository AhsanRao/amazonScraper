...
class AmazonSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        # Rankings - UPDATED WITH NEW STRUCTURE
        ...
        current_seller = {
            ...
            'prime': item.get('IsPrime'),
            'ships_from': seller_info.get('DispatchesFrom'),
            'fulfilled_by': seller_info.get('FulfilledBy'),
            'sold_by': seller_info.get('SoldBy'),
            'rating': item.get('StarRating'),
            'num_ratings': item.get('NumberOfRatings')
        }
        
        current_seller = {k: v for k, v in current_seller.items() if v is not None}
        item['Sellers'] = [current_seller] if current_seller else []
    ...
    def extract_brand(self, response):
        """Extract brand name - CLEANED VERSION"""
        brand_selectors = [
            '#bylineInfo::text',
            '#bylineInfo a::text',
            '.a-offscreen::text',
    ...
    def extract_listing_date(self, response):
        """Extract listing date - UPDATED WITH ISO FORMAT"""
    ...
    def extract_total_bought(self, response):
        """Extract total bought information - RETURNS INTEGER"""
        ...
        for selector in total_bought_selectors:
            ...
            if bought_text:
                ...
                if number_match:
                    return self.convert_sales_to_integer(number_match.group(1))
        ...
        for pattern in bought_patterns:
            ...
            if match:
                return self.convert_sales_to_integer(match.group(1))
        
        return None
    
    def convert_sales_to_integer(self, sales_text):
        """Convert sales text like '3K+' to integer 3000"""
        if not sales_text:
            return None
        
        try:
            clean_text = sales_text.replace('+', '')
